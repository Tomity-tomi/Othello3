...
public class OthelloMain extends JFrame {
    ...
    final static int blackAllottedTime = -1; // 黒プレーヤーの持ち時間 [ミリ秒] 1分 = 60000
					     // msec　負の値を設定すると時間無制限になる
    final static int whiteAllottedTime = -1; // 白プレーヤーの持ち時間 [ミリ秒] 1分 = 60000
					     // msec　負の値を設定すると時間無制限になる
	...
	try {
		...
		if (game.getCurrentPlayer() == Player.Black) { // 黒の手番
		    ...
		    while (!game.putPiece(blackStrategy.nextMove(
			    game.getGameState(),
			    game.getRemainingTime(Player.Black))))
		    ...
		    while (!game.putPiece(whiteStrategy.nextMove(
			    game.getGameState(),
			    game.getRemainingTime(Player.White))))
