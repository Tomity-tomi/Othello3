...
public class KaihoStrategy extends Strategy {
	...
	for (int i = 0; i < SIZE; i++) {
		...
		if ((expectedState.cells[i][j].getValue() - currentState.cells[i][j]
			.getValue())
			* (expectedState.cells[i][j].getValue() - currentState.cells[i][j]
				.getValue()) == 4) {
		    for (int k = i - 1; k <= i + 1; k++) {
			for (int l = j - 1; l <= j + 1; l++) {
			    if (currentState.isWithinRange(k, l) == false) {
				continue;
			    }
			    if (currentState.isEmpty(k, l) == true) {
